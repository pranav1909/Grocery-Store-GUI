...
class Database:
	def __init__(self,master):
		...
		self.btn_add = Button(master, text="Add to the Database",width=25,height=2,bg='steelblue',fg='white',command=self.get_items)
		self.btn_add.place(x=530,y=420)
		
		self.btn_clear = Button(master,text="Clear All Fields",width=18,height=2,bg='lightgreen',fg='white',command=self.clear_add)
	...
	def get_items(self):
		...
		if self.name == '' or self.stock == '' or self.cp == '' or self.sp == '':
			tkinter.messagebox.showinfo("Error", "Please fill the required entries.")
		else:
